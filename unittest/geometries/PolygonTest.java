...
class PolygonTest {
    ...
    void testFindIntersections() {
        ...
        final var result01 = polygon.findIntersections(new Ray(new Point(-0.7,0.5,0), Vector.AXIS_Z));
        ...
        assertNull(polygon.findIntersections(new Ray(new Point(-2,0.5,-2), Vector.AXIS_Z)),
                "Ray outside polygon against edge");

        // TC03: Ray is outside the polygon against vertex (0 points)
        assertNull(polygon.findIntersections(new Ray(new Point(-2,3,-2), Vector.AXIS_Z)),
                "Ray outside polygon against vertex");

        // =============== Boundary Values Tests ==================
        // TC11: Ray is on the edge
        assertNull(polygon.findIntersections(new Ray(new Point(-0.5,0.5,0), Vector.AXIS_Z)),
                "Ray on edge");

        // TC12: Ray is on the vertex
        assertNull(polygon.findIntersections(new Ray(new Point(0,0,0.5), Vector.AXIS_Z)),
                "Ray on vertex");

        // TC13: Ray is on the edge's continuation
        assertNull(polygon.findIntersections(new Ray(new Point(-2,2,-2), Vector.AXIS_Z)),
    ...
    void testCalculateIntersections() {
        ...
        assertNull(polygon.calculateIntersections(new Ray(new Point(-3, 0, 0), Vector.AXIS_X), 2),
                "ray stops before the polygon");

        // TC02: Ray crosses the polygon
        final var result02 = polygon.calculateIntersections(
        ...
        assertNull(polygon.calculateIntersections(new Ray(new Point(1, 0, 0), Vector.AXIS_X), 2),
                "ray starts after the polygon");

        // =============== Boundary Values Tests ==================
        // TC11: Ray "stops" at the polygon
        final var result11 = polygon.calculateIntersections(
